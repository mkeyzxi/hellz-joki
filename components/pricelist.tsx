...
export function Pricelist() {
  ...
  const [endStar, setEndStar] = useState(0);
      ...
      <div className="bg-(--surface-elevated) p-5 rounded-xl border border-(--border) flex-1 shadow-inner">
        ...
        <div className="space-y-4">
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-(--text-secondary) uppercase tracking-wider">Rank</label>
            <select 
              ...
              onChange={e => {
                ...
                if (newRank.type === 'romawi') {
                  ...
                  setDiv('');
                  setStar(newRank.minStars || 0);
                }
              }}
            >
              {pricelist.map(p => <option key={p.rank} value={p.rank}>{p.rank}</option>)}
            </select>
          </div>
          ...
          {selectedRankConfig.type === 'romawi' ? (
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                >
                  {selectedRankConfig.divisions!.slice().reverse().map(d => <option key={d} value={d}>{d}</option>)}
                </select>
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-(--text-secondary) uppercase tracking-wider">Bintang</label>
                <select 
                  className="w-full bg-(--background) border border-(--border) rounded-lg px-3 py-2.5 text-(--text-primary) focus:border-(--primary) focus:ring-1 focus:ring-(--primary) outline-none transition-all appearance-none cursor-pointer"
                  value={starInput}
                  onChange={e => setStar(parseInt(e.target.value))}
                >
                  {Array.from({ length: selectedRankConfig.starsPerDivision! }).map((_, i) => (
                    <option key={i} value={i}>{i} Bintang</option>
                  ))}
                </select>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-(--text-secondary) uppercase tracking-wider">Total Bintang</label>
              <input 
                type="number"
                min={selectedRankConfig.minStars}
                max={selectedRankConfig.maxStars !== 9999 ? selectedRankConfig.maxStars : undefined}
  ...
  const constructOrderText = () => {
    const startText = pricelist.find(r => r.rank === startRank)?.type === 'romawi' 
      ? `${startRank} ${startDiv} (${startStar} Bintang)`
      : `${startRank} (${startStar} Bintang)`;
    
    const endText = pricelist.find(r => r.rank === endRank)?.type === 'romawi'
      ...
      <div className="container mx-auto px-6 md:px-12 max-w-7xl">
          ...
          {pricelist.map((item, idx) => (
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-6 w-full relative z-10">
                <div className="w-14 h-14 relative shrink-0 bg-(--background) rounded-xl p-2 border border-(--border)/50 group-hover:border-(--primary)/30 transition-colors">
                  <Image src={item.icon} alt={item.rank} fill className="object-contain p-1 group-hover:scale-110 transition-transform duration-500" />
                </div>
                <div className="flex-1">
                  <h3 className="font-heading text-base md:text-lg text-(--text-primary) tracking-wide line-clamp-1">{item.rank}</h3>
                  <div className="flex items-baseline gap-1 mt-0.5">
                    <span className="text-xl md:text-2xl font-bold text-(--primary)">Rp{(item.price / 1000).toFixed(0)}k</span>
                    <span className="text-[10px] text-(--text-muted) uppercase tracking-wider">/ Bintang</span>
                  </div>
                </div>
              </div>
        ...
        >
          <div className="text-center mb-10">
            <h3 className="font-heading text-3xl text-(--text-primary) tracking-wide mb-2">Kalkulator Estimasi Biaya</h3>
            <p className="text-(--text-secondary) text-sm">Hitung secara akurat biaya joki dari titik awal ke tujuanmu.</p>
          </div>
          
          <div className="flex flex-col md:flex-row gap-6 mb-8">
            {renderSelector("Posisi Awal", startRank, startDiv, startStar, setStartRank, setStartDiv, setStartStar)}
            {renderSelector("Target Tujuan", endRank, endDiv, endStar, setEndRank, setEndDiv, setEndStar)}
          </div>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between p-6 bg-(--background) rounded-xl border border-(--border)/50">
            <div className="text-center md:text-left mb-6 md:mb-0">
              <p className="text-sm font-medium text-(--text-secondary) uppercase tracking-wider mb-1">Estimasi Total Biaya</p>
            ...
            <Button size="lg" className="w-full md:w-auto h-14 px-8 text-lg" disabled={!isValid} asChild>
              {isValid ? (
                <a href={`https://wa.me/62895326945192?text=${constructOrderText()}`} target="_blank" rel="noopener noreferrer">
                  Lanjut Order
                </a>
              ) : (
